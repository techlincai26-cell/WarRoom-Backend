...
type AssessmentService struct {
	...
	// reportLocks serialises GenerateReport calls per assessment so the multiple
	// async paths (Buyout, Walkout, Q_3_DECISION_BUYOUT, SimCompleted) can't race
	// and create duplicate report rows.
	reportLocks sync.Map
}
...
func (s *AssessmentService) SubmitResponse(assessmentID string, questionID string, responseData json.RawMessage) (*SubmitResponseResult, error) {
	...
	// Store previous responses for AI context (chronological, capped)
	s.updatePreviousResponses(assessmentID, question.Text, responseData, proficiency)

	// Source-of-truth: latest answer per question, used by dynamic scenarios,
	// the report endpoint, and continue-mode restart.
	upsertLatestResponse(assessmentID, stageID, questionID, responseData, proficiency)
	...
	if selectedOptionID == "Q_3_DECISION_BUYOUT" {
		...
		// Background report generation; the lock in GenerateReport dedupes with
		// any subsequent HandleBuyout / GetReport call so we never get duplicates.
		go func() {
			if _, err := s.GenerateReport(assessmentID); err != nil {
				log.Printf("[Assessment] Q_3_DECISION_BUYOUT report gen failed for %s: %v", assessmentID, err)
			}
		}()
		return result, nil
	}
...
func (s *AssessmentService) GetDynamicScenario(assessmentID string, stageID string, questionID string) (*models.DynamicScenario, error) {
	...
	// Build the AI's context preferring the LatestResponses map (per-question current
	// picks) when present, falling back to PreviousResponses (chronological summary)
	// for older assessments that pre-date the LatestResponses field.
	previousResponsesStr := s.buildContextForDynamicScenario(&assessment)
...
func (s *AssessmentService) SubmitDynamicScenario(assessmentID string, scenarioID string, selectedOptionID string) (*SubmitResponseResult, error) {
	...
	// Record the latest pick for this dynamic-scenario question. SubmitResponse will
	// also write it, but doing it here ensures downstream code reading LatestResponses
	// for branching never lags behind a successful dynamic-scenario submission even
	// if SubmitResponse short-circuits (e.g. assessment-not-in-progress).
	upsertLatestResponse(assessmentID, scenario.StageID, scenario.QuestionID, respData, selectedOpt.Proficiency)
...
func (s *AssessmentService) SubmitStageResponses(assessmentID string, responses map[string]json.RawMessage) (*SubmitStageResult, error) {
	...
	// Track the latest answer per question for downstream consumers
	// (reports, dynamic scenarios, restart-continue).
	for qID, out := range responseResults {
...
// GenerateReport produces (or returns cached) the FINAL evaluation report for an
// assessment. It is safe to call at any phase — buyout, walkout, mid-phase, or full
// completion — because it consumes whatever competency scores and responses exist
// at the moment of the call. Concurrent calls for the same assessment are serialised
// via reportLocks so the multiple async generation paths can't produce duplicates.
func (s *AssessmentService) GenerateReport(assessmentID string) (*models.Report, error) {
	return s.generateReportInternal(assessmentID, false)
}

// RegenerateReport forces a fresh report even if one already exists. Used after the
// user makes late changes (e.g. answers more questions after a draft report was
// auto-generated by buyout). The old report row is deleted before regeneration.
func (s *AssessmentService) RegenerateReport(assessmentID string) (*models.Report, error) {
	return s.generateReportInternal(assessmentID, true)
}

func (s *AssessmentService) generateReportInternal(assessmentID string, regenerate bool) (*models.Report, error) {
	// Per-assessment lock: ensures that two concurrent buyout / walkout / phase-submit
	// goroutines can't both attempt to insert a FINAL report row at the same time.
	lockVal, _ := s.reportLocks.LoadOrStore(assessmentID, &sync.Mutex{})
	...
	if err == nil {
		...
		// Regeneration requested — drop the prior row(s) before rebuilding.
		if delErr := db.DB.Where("assessmentId = ? AND reportType = ?", assessmentID, "FINAL").Delete(&models.Report{}).Error; delErr != nil {
			return nil, fmt.Errorf("failed to clear existing report for regenerate: %w", delErr)
		}
	} else if !errors.Is(err, gorm.ErrRecordNotFound) {
	...
	// exitedVia tells the UI how the user reached the report so it can pick the
	// right narrative ("Buyout at <stage>", "Walked out of war room", or "Completed
	// full simulation"). This makes the report endpoint truly phase-agnostic — a
	// buyout in phase 1 produces a coherent report without empty war-room sections.
	exitedVia := "COMPLETED"
	if assessment.BuyoutChosen {
		exitedVia = "BUYOUT"
	} else if assessment.Status == "COMPLETED" && len(scorecards) == 0 {
		// Status flipped to COMPLETED but no war-room interactions — early exit via
		// SubmitPhase finishing the last stage without a war room round.
		exitedVia = "EARLY_EXIT"
	} else if assessment.Status == "COMPLETED" && len(scorecards) > 0 {
		// Treat as walkout when no deal was accepted.
		hasAccepted := false
	...
	dealSummary := map[string]interface{}{
		...
		"exitedVia":       exitedVia,
		"exitStage":       assessment.CurrentStage,
		"capitalSource":   assessment.CapitalSource,
...
// Restart modes — RestartAssessment accepts these to support the PRD's
// "continue with current plan" vs "start from month 0 and replan" branching.
const (
	// RestartModeMonthZero performs a full wipe: deletes all responses, stages,
	// scorecards, mentor interactions, and resets all simulation state. The user
	// re-enters Ideation as if it were a fresh assessment.
	RestartModeMonthZero = "month_zero"

	// RestartModeContinue is a soft restart: preserves all prior responses and
	// competency scores, but resets the assessment to IN_PROGRESS and jumps the
	// cursor back to the requested stage's first question. Use this when the user
	// wants to revisit an earlier phase with their existing plan intact.
	RestartModeContinue = "continue"
)

// RestartAssessment restarts an assessment. With mode="month_zero" (default) it
// performs a full wipe and returns to Ideation. With mode="continue" it preserves
// prior answers and only resets the cursor — optionally to a specific stage.
//
// The legacy single-argument signature still works (defaults to month_zero) so
// existing callers and serialized restart endpoints don't need to change.
func (s *AssessmentService) RestartAssessment(assessmentID string, opts ...RestartOpts) (*models.Assessment, error) {
	mode := RestartModeMonthZero
	targetStage := ""
	if len(opts) > 0 {
		if opts[0].Mode != "" {
			mode = opts[0].Mode
		}
		targetStage = opts[0].TargetStage
	}
	...
	if mode == RestartModeContinue {
		return s.restartContinue(&assessment, targetStage, now)
	}

	return s.restartMonthZero(&assessment, now)
}

// RestartOpts is the optional argument bag for RestartAssessment. Kept as a struct
// so we can add fields (e.g. "preserveCharacters") without breaking callers.
type RestartOpts struct {
	Mode        string // RestartModeMonthZero or RestartModeContinue
	TargetStage string // optional — for continue mode, which stage to jump to (defaults to current)
}

func (s *AssessmentService) restartMonthZero(assessment *models.Assessment, now time.Time) (*models.Assessment, error) {
	assessmentID := assessment.ID
	firstStage := "STAGE_NEG2_IDEATION"
	firstQ := s.DataManager.GetFirstQuestionInStage(firstStage)
	...
	assessment.LatestResponses = nil
	...
	// Dynamic scenarios cached against the old responses are now stale.
	db.DB.Where("assessment_id = ?", assessmentID).Delete(&models.DynamicScenario{})
	// Drop any prior reports too — the user is starting over.
	db.DB.Where("assessmentId = ?", assessmentID).Delete(&models.Report{})
	...
	log.Printf("[Assessment] Restarted (month_zero): id=%s, count=%d", assessment.ID, assessment.RestartCount)
	return assessment, nil
}

// restartContinue performs a soft restart: prior responses, competency scores, and
// LatestResponses are preserved; only the assessment cursor is reset (and Status
// flipped back to IN_PROGRESS so the user can resume). If targetStage is empty,
// we default to the user's current stage so they re-enter where they were.
func (s *AssessmentService) restartContinue(assessment *models.Assessment, targetStage string, now time.Time) (*models.Assessment, error) {
	if targetStage == "" {
		targetStage = assessment.CurrentStage
	}
	stageData := s.DataManager.GetStage(targetStage)
	if stageData == nil {
		// Caller asked for a stage the data manager doesn't know — fall back to ideation.
		targetStage = "STAGE_NEG2_IDEATION"
		stageData = s.DataManager.GetStage(targetStage)
	}

	firstQ := s.DataManager.GetFirstQuestionInStage(targetStage)

	assessment.RestartCount++
	assessment.CurrentStage = targetStage
	assessment.CurrentQuestionID = firstQ
	if stageData != nil && len(stageData.SimulatedMonths) > 0 {
		assessment.SimulatedMonth = stageData.SimulatedMonths[0]
	}
	assessment.Status = "IN_PROGRESS"
	assessment.CompletedAt = nil
	assessment.LastActiveAt = &now
	// Clear the buyout flag so the user can complete the simulation properly this time.
	assessment.BuyoutChosen = false
	// Drop any cached report — a continue-restart implies the user wants a fresh
	// report once they finish the new path.
	db.DB.Where("assessmentId = ?", assessment.ID).Delete(&models.Report{})

	if err := db.DB.Save(assessment).Error; err != nil {
		return nil, fmt.Errorf("failed to continue-restart assessment: %w", err)
	}

	log.Printf("[Assessment] Restarted (continue): id=%s, count=%d, stage=%s", assessment.ID, assessment.RestartCount, targetStage)
	return assessment, nil
}
...
func (s *AssessmentService) AcceptDeal(assessmentID string, investorID string, capital float64, equity float64) (*models.Assessment, error) {
	...
	// Defensive: investor lookup can miss (e.g. fallback investor IDs or stale
	// frontend payloads) — fall back to the raw ID so we never log "for X.X%" with
	// an empty name.
	investorName := investorID
	if inv, ok := s.DataManager.InvestorMap[investorID]; ok && inv.Name != "" {
		investorName = inv.Name
	}
	assessment.CapitalSource = fmt.Sprintf("War Room Deal: %s for %.1f%%", investorName, equity)
...
// HandleBuyout marks the assessment as completed via buyout and synchronously
// generates the final report so the caller can navigate straight to it. The
// CurrentStage is left at whatever phase the user was on when they accepted the
// buyout (we no longer fake-advance it to STAGE_4_WARROOM, which was misleading
// — the user did NOT complete the war room phase).
func (s *AssessmentService) HandleBuyout(assessmentID string, company string, amount float64) (*models.Assessment, error) {
	...
	now := time.Now()
	assessment.BuyoutChosen = true
	// Keep CurrentStage as the user's actual exit point so the report can show
	// "exited via buyout at <stage>". CurrentQuestionID cleared so the simulation
	// UI doesn't try to render a question after we navigate away.
	...
	// Synchronously generate the report. The lock inside GenerateReport ensures
	// this is idempotent if a concurrent path (e.g. Q_3_DECISION_BUYOUT option in
	// SubmitResponse) also triggers generation. We swallow the error here because
	// the buyout itself succeeded — the report can always be retried via the
	// /assessments/:id/report GET endpoint.
	if _, err := s.GenerateReport(assessmentID); err != nil {
		log.Printf("[Assessment] Buyout report generation failed for %s: %v", assessmentID, err)
	}
...
// HandleWalkout marks the assessment as completed because the user rejected all
// investor offers. The report is generated synchronously (same rationale as
// HandleBuyout) so the UI can navigate straight to it.
func (s *AssessmentService) HandleWalkout(assessmentID string) (*models.Assessment, error) {
	...
	now := time.Now()
	// Stay on STAGE_4_WARROOM — the user did reach and complete the war-room phase
	// (they just declined all offers), so this is accurate.
	...
	if _, err := s.GenerateReport(assessmentID); err != nil {
		log.Printf("[Assessment] Walkout report generation failed for %s: %v", assessmentID, err)
	}
